...
async function main() {
    ...
    const { data: shifts } = await supabase.from('shifts').select('*').in('employee_id', empIds).in('shift_date', ["2026-03-13", "2026-03-14", "2026-03-15"]).gt('hours_worked', 0);

    for (const targetDate of ["2026-03-14", "2026-03-15"]) {
        const yesterdayDate = targetDate === "2026-03-14" ? "2026-03-13" : "2026-03-14";
        const hourlyCounts = new Array(24).fill(0);

        for (const emp of emps) {
            if (!emp.name || emp.is_active === false) continue;
            const empShifts = shifts.filter(s => s.employee_id === emp.id);
            const yShift = empShifts.find(s => s.shift_date === yesterdayDate);
            const tShift = empShifts.find(s => s.shift_date === targetDate);
            ...
            if (tShift) {
                ...
                if (nC) {
                    ...
                    if (tShift.hours_worked * 60 > nD + 120) {
                        let e = eM; if (e <= sM) e += 1440;
                        intervals.push({ s: sM, e: e });
                    }
                } else {
                    let e = eM;
                    let isPartial = tShift.hours_worked < 8 || note.toLowerCase().includes("доп");
                    if (isPartial) {
                        e = sM + (tShift.hours_worked * 60);
                    } else if (e <= sM) {
                        e += 1440;
                    }
                    intervals.push({ s: sM, e: e });
            ...
            for (const iv of intervals) {
                ...
                for (let h = 0; h < 24; h++) {
                    ...
                    if (cm >= rs && cm < re) {
                        hourlyCounts[h]++;
                        if (h === 0) {
                            console.log(`[${targetDate}] Hour 0 covered by ${emp.name}. yShift: ${!!yShift}, tShift: ${!!tShift}. Intervals:`, JSON.stringify(intervals));
                        }
                    }
                }
            }
        ...
        console.log(`[${targetDate}] Hourly counts:`, hourlyCounts);
    }
}
